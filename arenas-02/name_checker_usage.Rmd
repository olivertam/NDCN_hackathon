...
We've created this code after the Arenas lab talked together about
what their current, personal naming conventions are, and what nice lab
conventions could be. But this is a very iterative process to align
and decide on the conventions and write the code that works just like you want.

These are some updates/iterations that we can discuss, and can also look at the code of what we would do to change it: 
...
- **Checking multiple folders**. The function currently operates on just one folder you provide. But you can add multiple!
...
While it might seem that the function is designed specifically for the Arenas lab, there
are a few features that could be re-used for other code.
...
What it is looking for in the example above are items (in this case, files) that end in
".czi". 
- The "." is represented as `\\.` (it's because both back-slashes
  (`\`) and dots (`.`) have special meaning in regular expression, so you have to use
  a weird combination of them to make the computer understand exactly
  what you want)
...
secondary:
`goPITX3-dk488` becomes `goPITX3` and `dk488`.
THe function then extracts the first two letter to get the host species ID (e.g. `go`),
and treats the rest as the target (gene symbol or fluorphore). As we
discussed above, it capitalizes the gene symbol if required.
...
might want to check what it's actually finding. The code also has a way to do this by invoking the
`verbose` option:
