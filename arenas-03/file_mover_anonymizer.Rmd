...
We will work from this `file_mover.Rmd` document today. It is
...
The goal of the script move them into the `moved` subfolder (which is
the "root" of the directory tree), and then sort into further
subfolders ("branches") corresponding to the file name. 

First, we want to set up a folder for logging our command. The
...
2. `dest_root`: the "root" folder where you want them to go
...
### What is in the code
if(! dir.exists(folder)){
    log = c(log,paste0(c(paste("This folder", folder, "does not exist."),"Please double-check","")))
}

### What happens when we remove the '!'
if(dir.exists(folder)){
    log = c(log(paste(c(paste("This folder", folder, "actually exist."),"Please double-check","")))
}
cat(log, sep="\n")
```
...
Then, for each file, I get the extension (e.g. `.tif`), and then
"randomly" choose a number from the counter list (without replacement)

```{r choose_without_replacement, eval=FALSE}
if(length(counter) == 1){
    ### Only 1 number left
    filenum <- counter
}else{
...
It then generates a "anonymized" filename starting with
`anonymizedFile`, current date, the random number (separated by
