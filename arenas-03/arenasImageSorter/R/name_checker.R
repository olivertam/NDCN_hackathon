...
#'
#' This function will check that the image files are named according
#' to the convention outlined by the Arenas lab.
#'
#' @param folder Folder containing the image files
#' @param versbose Generate a verbose log (default = FALSE)
#' @param print2screen Print log to STDOUT (default = TRUE)
#' @return A log indicating if there were issues with the file name
#' @export
...
name_checker <- function(folder, verbose = FALSE, print2screen = TRUE){ 
    ...
    extract_information <- function(name){
        ...
        if(is.na(as.Date(file_info[4],date.format))){
            ...
            if(verbose){
                output = c(output,paste("Immunostaining date:",file_info[4]))
            }
        }
