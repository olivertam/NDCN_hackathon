...
Navigate to the "hackathon-arenas-02" folder.
...
If you have trouble with the `here` package, you can run the following:

```{r source_no_here eval=FALSE}
source(file.path("hackathon-arenas-02","code", "name_checker.R"))
```

This code gives you a list of parameters that the function would take:
...
that should be checked. The function that finds all the image files
...
There are 4 TIFF files in `files_test`, which are named loosely according to
the Arenas lab convention. We should check if they are named correctly.

```{r oliver_test}
results <- name_checker(here("hackathon-arenas-02",'files_test'))
```

```{r oliver_test_no_here eval=FALSE}
results <- name_checker(file.path("hackathon-arenas-02",'files_test'))
```
...
sections. The function will print out this information so the user can double-check.
...
```{r verbose}
results  <- name_checker(here("hackathon-arenas-02","files_test","Good_file"),verbose=TRUE)
```

```{r verbose_no_here}
results  <- name_checker(file.path("hackathon-arenas-02","files_test","Good_file"),verbose=TRUE)
```
...
```{r carmen_test}
results <- name_checker(here("hackathon-arenas-02","files_carmen"))
```

```{r carmen_test_no_here}
results <- name_checker(file.path("hackathon-arenas-02","files_carmen"))
```
...
results <- name_checker(file.path("hackathon-arenas-02","files_test","Good_file"))
```
