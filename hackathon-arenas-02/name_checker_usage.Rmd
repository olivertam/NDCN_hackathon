...
```{r oliver_test}
results <- check_file_names(here('files_oliver'))
```
...
should go to: `NES-SAI2d15-CS/200514-01`
This is part of the Arenas lab's workflow, where they were hoping to
...
```{r verbose}
results  <- check_file_names(here('files_oliver/Good_file'))
```
...
```{r julie_test}
results <- check_file_names(here('files_julietest'))
```
